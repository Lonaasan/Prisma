...
function prismaStorage.get(key)
    if not key then
        prismaError.panic("Key is nil");
    end
    if storage[key] == nil then
        prismaError.panic("Key does not exist");
    end
    return storage[key];
end
...
function prismaStorage.getInTag(tag, key)
    if not tag or not key then
        prismaError.panic("Tag or key is nil");
    end
    if storage[tag] == nil then
        prismaError.panic("Tag does not exist");
    end
...
function prismaStorage.post(key, value)
    if not key or not value then
        prismaError.panic("Key or value is nil");
    end
    if storage[key] then
        prismaError.panic("Key already exists");
    end
    storage[key] = value;
end
...
function prismaStorage.postInTag(tag, key, value)
    if not tag or not key or not value then
        prismaError.panic("Tag or key or value is nil");
    end
    if storage[tag] == nil then
        prismaError.panic("Tag does not exist");
    end
