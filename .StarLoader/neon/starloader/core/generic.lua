...
function update(...)
	...
	for i = 1, #funcs do
		...
		if canrun == false and firstupdate then
			sb.logWarn("\n["
            .. mainconfig.logname ..
            "] [%s] Module dependency missing or could not load!\n["
            .. mainconfig.logname ..
            "] [%s] Please check your modules.json or .patch file.",
            modulefuncs.modulename[i], modulefuncs.modulename[i]);
		end;
	end;
	init, update, uninit = _init, _update, _uninit;
	firstupdate = false;
...
function sl.loadConfig()
	if thirdpartyenabled then
		local loaded, _ = pcall(sl.loadHasibound());
		if loaded == false then
			sb.logWarn("\n["
            .. mainconfig.logname ..
            "] [THIRDPARTY] Third-Party Mod could not load!\n["
            .. mainconfig.logname ..
            "] [THIRDPARTY] Please check /neon/starloader/core/config.json");
		end;
	end;
	sb.logInfo("\n[" .. mainconfig.logname .. "] [INIT] Config loaded!");
end;
function sl.searchModule(name)
	if moduleconf.modules[name] then
		return moduleconf.modules[name].path;
	end;
end;
function sl.getModules()
